...
class PatchTrainer:
    ...
    def __init__(self, cfg):
        ...
        detect_model = YOLOv10('yolov10/ckpt/yolov10s.pt')
        imgsz = 640
        _ = detect_model(torch.zeros(1, 3, imgsz, imgsz).to(self.dev).type_as(next(detect_model.parameters())), verbose=False)
    ...
    def yolov102yolov5(self, prediction): # output b x nums x 85
        pred1 = prediction[0]['one2one'][0]
        pred2 = prediction[0]['one2many'][0]
        pred = torch.cat((pred1, pred2), dim=2).permute(0, 2, 1)
        boxes_yolov10 = pred[:, :, :4]
        label_prob = pred[:, :, 4:]
        conf_yolov10, _ = torch.max(label_prob, dim=2, keepdim=True)
        return torch.cat((boxes_yolov10, conf_yolov10, label_prob), dim=2)
    ...
    def train(self):
        ...
        hook = self.detect_model.model.model[-1].register_forward_hook(hookfn)
            ...
            for i_batch, (img_batch, lab_batch) in tqdm(enumerate(self.train_loader),
                ...
                with autograd.set_detect_anomaly(mode=True):
                    ...
                    with autocast() if self.cfg.use_amp else nullcontext():
                        output = []
                        self.detect_model(p_img_batch, verbose=False)
                        
                        pred = self.yolov102yolov5(output)
    ...
    def val(self, epoch: int, patchfile: str, log_dir: str, conf_thresh: float = 0.5, nms_thresh: float = 0.5) -> None:
        ...
        output = []
        def hookfnval(modul, inf, outf):
            output.append(outf)
    
        hookval = self.detect_model.model.model[-1].register_forward_hook(hookfnval)
        ...
        for imgfile in tqdm(img_paths, desc=f'Running val epoch {epoch}'):
            ...
            output = []
            self.detect_model(padded_img_tensor, verbose=False)
            pred = self.yolov102yolov5(output)

            # tv = self.tv_loss(adv_patch) if self.cfg.tv_mult != 0 else zero_tensor
            # max_prob = self.prob_extractor(output)
            # pred = self.yolov102yolov5(pred)
            ...
            output = []
            self.detect_model(p_tensor_batch, verbose=False)
            pred = self.yolov102yolov5(output)
